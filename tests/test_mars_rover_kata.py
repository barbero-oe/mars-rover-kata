...
from mars_rover_kata.planet import Planet, PlanetMap
from mars_rover_kata.rover import Rover
from tests.position_builder import initial_position, position_at


@pytest.mark.parametrize('cardinal_point,coordinates',
                         [('N', (0, 1)), ('S', (0, -1)), ('E', (1, 0)), ('W', (-1, 0))])
def test_rover_moves_forward(cardinal_point, coordinates):
    rover = given_rover_at_map_center_facing(cardinal_point)
...
def given_rover_at_map_center_facing(cardinal_point):
    planet_map = PlanetMap((10, 10))
    return Rover(position_at((5, 5, cardinal_point)).build(), planet_map)
